function [sol_struct] = Z_proxSG(F,prox_r,Xi,mu,x_0,T,alpha,k,plot,F_plot)
% ================================================================================== %
% Zeroth-order Proximal Stochastic Gradient method.
% ---------------------------------------------------------------------------------- %
% ** [sol_struct] = Z_proxSG(F,prox_r,mu,x_0,T,alpha,k) **
%
...
% surrogate problem
%
% \[          min_{x \in R^n}  f_{\mu}(x) + r(x),               (P_{mu})         \]
%
% where $f_{\mu}()$ is a smoothed surrogate for $f()$ obtained via the 
% use of Gaussian smoothing. The method, at each iteration $j$
%
%   • generates a sample-vector $\xi$ from the (unknown) distribution $P$ 
%     on which $F(x,\xi)$ depends
%   • generates $k$ sample-vectors from a standard normal distribution N(0,I_n)
%   • evaluates $F(x,\xi)$ as well as $F(x+\mu*U_i,\xi)$, i = 1,...,k
%   • computes an unbiased gradient estimate for $f_{\mu}(), as:
%          $G(x,\xi) = 1/(k*\mu)*\sum_i((F(x+\mu U_i,\xi) - F(x,\xi))U_i)$
...
%   • mu     - the smoothing parameter for the Gaussian smoothing.
...
% __________________________________________________________________________________ %
    ...
    for t = 0:T
        ...
        for j = 0:k-1
            ...
            G = G + (((1/mu)*(F(x+ mu.*U(j*n+1:(j+1)*n,1),xi)-F_t)).*U(j*n+1:(j+1)*n,1));
        end
